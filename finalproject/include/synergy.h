...
extern char SHELL_OUTPUT[MAX_STDOUT];
...
#define TSH_OP_SHELL              405 // ignore everything after this
...
typedef struct { // Struct incoming to the server, sending length
   size_t length ;   // OP_SHELL input structure to tsh.c
} tsh_shell_it ;

typedef struct{ 
   sng_int16 status;
   sng_int16 error;
   sng_int32 stdoutLength; // Follow up read of real stdout
